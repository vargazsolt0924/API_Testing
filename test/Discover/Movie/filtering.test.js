...
describe(testData.filters.describe, () => {
  ...
  beforeAll(async () => {
    ...
    filteredByYear = await spec().get('/discover/movie').withQueryParams('primary_release_year', yearCase.query.primary_release_year).expectStatus(200).toss();

    filteredByGenre = await spec().get('/discover/movie').withQueryParams('with_genres', genreCase.query.with_genres).expectStatus(200).toss();
  });
